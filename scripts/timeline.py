...
class timeline:
    ...
    def add_ann(self):
        ...
        self.table = self.table.astype({'Pos': int})
        self.table['Locus Tag'] = self.table.apply(lambda x: self.gff.gff_pos[x['Chrom']][x['Pos']][0], axis=1)
        self.table['Gene'] = self.table.apply(lambda x: self.gff.gff_pos[x['Chrom']][x['Pos']][1], axis=1)
        self.table['ID'] = self.table.apply(lambda x: self.gff.gff_pos[x['Chrom']][x['Pos']][2], axis=1)
        self.table['Annotation'] = self.table.apply(lambda x: self.gff.gff_pos[x['Chrom']][x['Pos']][3], axis=1)
